...
(encapsulate
 ((dcc-fn1 (context x) t)
  (derivative-dcc-fn1 (context x) t)
  (dcc-fn2 (context x) t)
  (derivative-dcc-fn2 (context x) t)
  (dcc-fnz (context x) t)
  (derivative-dcc-fnz (context x) t)
  (dcc-fn-domain () t))

 ;; Our witness continuous function is the identity function.

 (local (defun dcc-fn1 (context x) (declare (ignore context x)) 1))
 (local (defun derivative-dcc-fn1 (context x) (declare (ignore context x)) 0))
 (local (defun dcc-fn2 (context x) (declare (ignore context x)) 1))
 (local (defun derivative-dcc-fn2 (context x) (declare (ignore context x)) 0))
 (local (defun dcc-fnz (context x) (declare (ignore context x)) 1))
 (local (defun derivative-dcc-fnz (context x) (declare (ignore context x)) 0))
 ...
 (defthm dcc-fn1-real
   (realp (dcc-fn1 context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm dcc-fn2-real
   (realp (dcc-fn2 context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm dcc-fnz-real
   (realp (dcc-fnz context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm dcc-fnz-non-zero
   (implies (inside-interval-p x (dcc-fn-domain))
	    (not (equal (dcc-fnz context x) 0)))
   :rule-classes (:rewrite :type-prescription))

 (defthm derivative-dcc-fn1-real
     (realp (derivative-dcc-fn1 context x))
     :rule-classes (:rewrite :type-prescription))

 (defthm derivative-dcc-fn2-real
     (realp (derivative-dcc-fn2 context x))
     :rule-classes (:rewrite :type-prescription))

 (defthm derivative-dcc-fnz-real
     (realp (derivative-dcc-fnz context x))
     :rule-classes (:rewrite :type-prescription))

 (defun-sk forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fn1 (context x eps delta)
   (forall (x1)
	   (implies (and (inside-interval-p x1 (dcc-fn-domain))
			 ...
			 (not (equal x x1))
			 (< (abs (- x x1)) delta))
		    (< (abs (- (/ (- (dcc-fn1 context x) (dcc-fn1 context x1)) (- x x1))
			       (derivative-dcc-fn1 context x)))
		       eps)))
   :rewrite :direct)

 (defun-sk forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fn2 (context x eps delta)
   (forall (x1)
	   (implies (and (inside-interval-p x1 (dcc-fn-domain))
			 (inside-interval-p x (dcc-fn-domain))
			 (realp delta)
			 ...
			 (not (equal x x1))
			 (< (abs (- x x1)) delta))
		    (< (abs (- (/ (- (dcc-fn2 context x) (dcc-fn2 context x1)) (- x x1))
			       (derivative-dcc-fn2 context x)))
		       eps)))
   :rewrite :direct)

 (defun-sk forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fnz (context x eps delta)
   (forall (x1)
	   (implies (and (inside-interval-p x1 (dcc-fn-domain))
			 (inside-interval-p x (dcc-fn-domain))
			 (realp delta)
			 ...
			 (not (equal x x1))
			 (< (abs (- x x1)) delta))
		    (< (abs (- (/ (- (dcc-fnz context x) (dcc-fnz context x1)) (- x x1))
			       (derivative-dcc-fnz context x)))
		       eps)))
   :rewrite :direct)

 (defun-sk exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn1 (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fn1 context x eps delta)))))

 (defun-sk exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn2 (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
			;(standardp x)
			(realp eps)
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fn2 context x eps delta)))))

 (defun-sk exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fnz (context x eps)
   (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
			;(standardp x)
			(realp eps)
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-classical-works-for-dcc-fnz context x eps delta)))))

 (defthm derivative-dcc-fn1-is-derivative
  (implies (and (inside-interval-p x (dcc-fn-domain))
		(realp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn1 context x eps))
  :hints (("Goal"
 ...
 (defthm derivative-dcc-fn2-is-derivative
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (realp eps)
		 (< 0 eps))
	    (exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn2 context x eps))
  :hints (("Goal"
  ...
  (defthm derivative-dcc-fnz-is-derivative
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (realp eps)
		 (< 0 eps))
	    (exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fnz context x eps))
  :hints (("Goal"
...
(defthm dcc-fn1-differentiable-using-nonstd-criterion
  (implies (and (standardp context)
	   ...
	   (and (i-limited (/ (- (dcc-fn1 context x) (dcc-fn1 context y1)) (- x y1)))
		(i-close (/ (- (dcc-fn1 context x) (dcc-fn1 context y1)) (- x y1))
			 (/ (- (dcc-fn1 context x) (dcc-fn1 context y2)) (- x y2)))))
  :hints (("Goal"
	  ...
	  ("Subgoal 5"
	   :cases ((and (inside-interval-p x (dcc-fn-domain))
			(realp eps)
			(< 0 eps)))
	   :expand ((exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn1 context x eps))
...
(defthm dcc-fn2-differentiable-using-nonstd-criterion
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited (/ (- (dcc-fn2 context x) (dcc-fn2 context y1)) (- x y1)))
		(i-close (/ (- (dcc-fn2 context x) (dcc-fn2 context y1)) (- x y1))
			 (/ (- (dcc-fn2 context x) (dcc-fn2 context y2)) (- x y2)))))
   :hints (("Goal"
	    :by (:functional-instance rdfn-classic-is-differentiable-using-nonstd-criterion
	   ...
	   ("Subgoal 5"
	    :cases ((and (inside-interval-p x (dcc-fn-domain))
			 (realp eps)
			 (< 0 eps)))
	    :expand ((exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fn2 context x eps))
...
(defthm dcc-fnz-differentiable-using-nonstd-criterion
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited (/ (- (dcc-fnz context x) (dcc-fnz context y1)) (- x y1)))
		(i-close (/ (- (dcc-fnz context x) (dcc-fnz context y1)) (- x y1))
			 (/ (- (dcc-fnz context x) (dcc-fnz context y2)) (- x y2)))))
   :hints (("Goal"
	    :by (:functional-instance rdfn-classic-is-differentiable-using-nonstd-criterion
	   ...
	   ("Subgoal 5"
	    :cases ((and (inside-interval-p x (dcc-fn-domain))
			 (realp eps)
			 (< 0 eps)))
	    :expand ((exists-delta-for-x-and-eps-so-deriv-classical-works-for-dcc-fnz context x eps))
...
(defthmd dcc-fn1-is-differentiable
  (implies (and (standardp context)
		...
		(not (equal x1 x)))
	   (i-close (/ (- (dcc-fn1 context x) (dcc-fn1 context x1)) (- x x1))
		    (derivative-dcc-fn1 context x)))
  :hints (("Goal"
...
(defthmd dcc-fn2-is-differentiable
  (implies (and (standardp context)
		...
		(inside-interval-p x1 (dcc-fn-domain))
		(not (equal x1 x)))
	   (i-close (/ (- (dcc-fn2 context x) (dcc-fn2 context x1)) (- x x1))
		    (derivative-dcc-fn2 context x)))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable
				     (rdfn-classical dcc-fn2)
...
(defthmd dcc-fnz-is-differentiable
  (implies (and (standardp context)
		...
		(inside-interval-p x1 (dcc-fn-domain))
		(not (equal x1 x)))
	   (i-close (/ (- (dcc-fnz context x) (dcc-fnz context x1)) (- x x1))
		    (derivative-dcc-fnz context x)))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable
				     (rdfn-classical dcc-fnz)
...
(local
 (defthm-std standard-derivative-dcc-fn1
   (implies (and (standardp x)
                 (standardp context))
	    (standardp (derivative-dcc-fn1 context x)))))

(local
 (defthm-std standard-derivative-dcc-fn2
   (implies (and (standardp x)
                 (standardp context))
	    (standardp (derivative-dcc-fn2 context x)))))

(local
 (defthm-std standard-derivative-dcc-fnz
   (implies (and (standardp x)
                 (standardp context))
	    (standardp (derivative-dcc-fnz context x)))))

(defun differential-dcc-fn1 (context x eps)
  (/ (- (dcc-fn1 context (+ x eps)) (dcc-fn1 context x)) eps))
...
(local
 (defthmd derivative-dcc-fn1-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (standardp x)
                 (standardp context))
	    (equal (derivative-dcc-fn1 context x)
		   (if (inside-interval-p (+ x (/ (i-large-integer))) (dcc-fn-domain))
		       (standard-part (differential-dcc-fn1 context x (/ (i-large-integer))))
		     (if (inside-interval-p (- x (/ (i-large-integer))) (dcc-fn-domain))
			 (standard-part (differential-dcc-fn1 context x (- (/ (i-large-integer)))))
		       'error))))
   :hints (("Goal"
	    :use ((:instance dcc-fn1-is-differentiable
		  ...
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fn1 context x))
			     (y (+ (- (* (i-large-integer)
					 (dcc-fn1 context x)))
				   (* (i-large-integer)
				      (dcc-fn1 context (+ (/ (i-large-integer)) x))))))
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fn1 context x))
			     (y (+ (* (i-large-integer)
				      (dcc-fn1 context x))
...
(defun differential-dcc-fn2 (context x eps)
  (/ (- (dcc-fn2 context (+ x eps)) (dcc-fn2 context x)) eps))
...
(local
 (defthmd derivative-dcc-fn2-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (standardp x)
                 (standardp context))
	    (equal (derivative-dcc-fn2 context x)
		   (if (inside-interval-p (+ x (/ (i-large-integer))) (dcc-fn-domain))
		       (standard-part (differential-dcc-fn2 context x (/ (i-large-integer))))
		     (if (inside-interval-p (- x (/ (i-large-integer))) (dcc-fn-domain))
			 (standard-part (differential-dcc-fn2 context x (- (/ (i-large-integer)))))
		       'error))))
   :hints (("Goal"
	    :use ((:instance dcc-fn2-is-differentiable
		  ...
		  (:instance i-close-to-small-sum
			     ...
			     (eps (- (/ (i-large-integer)))))
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fn2 context x))
			     (y (+ (- (* (i-large-integer)
					 (dcc-fn2 context x)))
				   (* (i-large-integer)
				      (dcc-fn2 context (+ (/ (i-large-integer)) x))))))
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fn2 context x))
			     (y (+ (* (i-large-integer)
				      (dcc-fn2 context x))
...
(defun differential-dcc-fnz (context x eps)
  (/ (- (dcc-fnz context (+ x eps)) (dcc-fnz context x)) eps))
...
(local
 (defthmd derivative-dcc-fnz-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (standardp x)
                 (standardp context))
	    (equal (derivative-dcc-fnz context x)
		   (if (inside-interval-p (+ x (/ (i-large-integer))) (dcc-fn-domain))
		       (standard-part (differential-dcc-fnz context x (/ (i-large-integer))))
		     (if (inside-interval-p (- x (/ (i-large-integer))) (dcc-fn-domain))
			 (standard-part (differential-dcc-fnz context x (- (/ (i-large-integer)))))
		       'error))))
   :hints (("Goal"
	    :use ((:instance dcc-fnz-is-differentiable
		  ...
		  (:instance i-close-to-small-sum
			     ...
			     (eps (- (/ (i-large-integer)))))
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fnz context x))
			     (y (+ (- (* (i-large-integer)
					 (dcc-fnz context x)))
				   (* (i-large-integer)
				      (dcc-fnz context (+ (/ (i-large-integer)) x))))))
		  (:instance close-x-y->same-standard-part
			     (x (derivative-dcc-fnz context x))
			     (y (+ (* (i-large-integer)
				      (dcc-fnz context x))
...
(encapsulate
 ...
 (defun dcc-fn1+fn2 (context x)
   (+ (dcc-fn1 context x) (dcc-fn2 context x)))

 (local
  (defun differential-dcc-fn1+fn2 (context x eps)
    (+ (differential-dcc-fn1 context x eps)
       (differential-dcc-fn2 context x eps))))

 (local
  (defun derivative-dcc-fn1+fn2 (context x)
    (+ (derivative-dcc-fn1 context x)
       (derivative-dcc-fn2 context x))))
 ...
 (defthm differential-dcc-fn1+fn2-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (differential-dcc-fn1+fn2 context x eps)
		   (+ (differential-dcc-fn1 context x eps)
		      (differential-dcc-fn2 context x eps)))))
 ...
 (defthm derivative-dcc-fn1+fn2-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (standardp x)
                 (standardp context))
	    (equal (derivative-dcc-fn1+fn2 context x)
		   (+ (derivative-dcc-fn1 context x)
		      (derivative-dcc-fn2 context x)))))
 )
...
(defthm dcc-fn1+fn2-differentiable
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited  (/ (- (dcc-fn1+fn2 context x) (dcc-fn1+fn2 context y1)) (- x y1)))
		(i-close (/ (- (dcc-fn1+fn2 context x) (dcc-fn1+fn2 context y1)) (- x y1))
			 (/ (- (dcc-fn1+fn2 context x) (dcc-fn1+fn2 context y2)) (- x y2)))))
  :hints (("Goal"
...
(defun-sk forall-x-eps-delta-in-range-deriv-dcc-fn1+fn2-works (context x eps delta)
  (forall (x1)
	  (implies (and (inside-interval-p x1 (dcc-fn-domain))
			(inside-interval-p x (dcc-fn-domain))
			(realp delta)
			...
			(not (equal x x1))
			(< (abs (- x x1)) delta))
		   (< (abs (- (/ (- (dcc-fn1+fn2 context x) (dcc-fn1+fn2 context x1)) (- x x1))
			      (derivative-dcc-fn1+fn2 context x)))
		      eps))))

(defun-sk exists-delta-for-x-and-eps-so-deriv-dcc-fn1+fn2-works (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
			(realp eps)
			(< 0 eps))
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-dcc-fn1+fn2-works context x eps delta)))))
...
(defthmd differential-dcc-fn1+fn2-close
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 ...
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (standard-part (differential-dcc-fn1+fn2 context x eps))
		   (derivative-dcc-fn1+fn2 context x)))
  :hints (("Goal"
...
(defthmd dcc-fn1+fn2-derivative-using-classical-criterion
  (implies (and (standardp x)
		...
		(standardp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-dcc-fn1+fn2-works context x eps))
  :hints (("Goal"
...
(encapsulate
 ...
 (defun dcc-minus-fn1 (context x)
   (- (dcc-fn1 context x)))

 (local
  (defun differential-dcc-minus-fn1 (context x eps)
    (- (differential-dcc-fn1 context x eps))))

 (local
  (defun derivative-dcc-minus-fn1 (context x)
    (- (derivative-dcc-fn1 context x))))

 (defthm differential-dcc-minus-fn1-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (differential-dcc-minus-fn1 context x eps)
		   (- (differential-dcc-fn1 context x eps)))))
 ...
 (defthm derivative-dcc-minus-fn1-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (standardp x)
                 (standardp context))
	    (equal (derivative-dcc-minus-fn1 context x)
		   (- (derivative-dcc-fn1 context x)))))
 )
...
(defthm dcc-minus-fn1-differentiable
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited  (/ (- (dcc-minus-fn1 context x) (dcc-minus-fn1 context y1)) (- x y1)))
		(i-close (/ (- (dcc-minus-fn1 context x) (dcc-minus-fn1 context y1)) (- x y1))
			 (/ (- (dcc-minus-fn1 context x) (dcc-minus-fn1 context y2)) (- x y2)))))
  :hints (("Goal"
...
(defun-sk forall-x-eps-delta-in-range-deriv-dcc-minus-fn1-works (context x eps delta)
  (forall (x1)
	  (implies (and (inside-interval-p x1 (dcc-fn-domain))
			(inside-interval-p x (dcc-fn-domain))
			(realp delta)
			...
			(not (equal x x1))
			(< (abs (- x x1)) delta))
		   (< (abs (- (/ (- (dcc-minus-fn1 context x) (dcc-minus-fn1 context x1)) (- x x1))
			      (derivative-dcc-minus-fn1 context x)))
		      eps))))

(defun-sk exists-delta-for-x-and-eps-so-deriv-dcc-minus-fn1-works (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
			(realp eps)
			(< 0 eps))
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-dcc-minus-fn1-works context x eps delta)))))
...
(defthmd differential-dcc-minus-fn1-close
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 ...
		 (realp eps) (i-small eps) (not (= eps 0))
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (standard-part (differential-dcc-minus-fn1 context x eps))
		   (derivative-dcc-minus-fn1 context x)))
  :hints (("Goal"
...
(defthmd dcc-minus-fn1-derivative-using-classical-criterion
  (implies (and (standardp context)
		...
		(standardp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-dcc-minus-fn1-works context x eps))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classical-derivative-using-classical-criterion
				     (rdfn dcc-minus-fn1)
				     (rdfn-domain dcc-fn-domain)
...
(defthm expand-differential-dcc-fn1*fn2
  (implies (and (inside-interval-p x (dcc-fn-domain))
		(inside-interval-p (+ x eps) (dcc-fn-domain)))
	   (equal (differential-dcc-fn1*fn2 context x eps)
		  (/ (- (dcc-fn1*fn2 context (+ x eps)) (dcc-fn1*fn2 context x)) eps)))
  :hints (("Goal"
...
(encapsulate
 ...
 (defun dcc-/-fnz (context x)
   (/ (dcc-fnz context x)))

 (local
  (defun differential-dcc-/-fnz (context x eps)
    (- (/ (differential-dcc-fnz context x eps)
	  (* (dcc-fnz context (+ x eps))
	     (dcc-fnz context x))))
    ))

 (local
  (defun derivative-dcc-/-fnz (context x)
    (- (/ (derivative-dcc-fnz context x)
	  (* (dcc-fnz context x)
	     (dcc-fnz context x))))
    ))
 ...
 (defthm differential-dcc-/-fnz-definition
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (differential-dcc-/-fnz context x eps)
		   (- (/ (differential-dcc-fnz context x eps)
			 (* (dcc-fnz context (+ x eps))
			    (dcc-fnz context x)))))))
...
(defun-sk forall-x-eps-delta-in-range-deriv-dcc-/-fnz-works (context x eps delta)
  (forall (x1)
	  (implies (and (inside-interval-p x1 (dcc-fn-domain))
			(inside-interval-p x (dcc-fn-domain))
			(realp delta)
...
(defun-sk exists-delta-for-x-and-eps-so-deriv-dcc-/-fnz-works (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (dcc-fn-domain))
			(realp eps)
			(< 0 eps))
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-dcc-/-fnz-works context x eps delta)))))
...
(defthmd differential-dcc-/-fnz-close
   (implies (and (inside-interval-p x (dcc-fn-domain))
		 ...
		 (realp eps) (i-small eps) (not (= eps 0))
		 (inside-interval-p (+ x eps) (dcc-fn-domain)))
	    (equal (standard-part (differential-dcc-/-fnz context x eps))
		   (derivative-dcc-/-fnz context x)))
  :hints (("Goal"
...
(defthmd dcc-/-fnz-derivative-using-classical-criterion
  (implies (and (standardp context)
		...
		(standardp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-dcc-/-fnz-works context x eps))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classical-derivative-using-classical-criterion
				     (rdfn dcc-/-fnz)
				     (rdfn-domain dcc-fn-domain)
...
(encapsulate
 ((ccr-fn1 (context x) t)
  (derivative-ccr-fn1 (context x) t)
  (ccr-fn2 (context x) t)
  (derivative-ccr-fn2 (context x) t)
  (ccr-fn2-domain () t)
  (ccr-fn2-range () t))

 ;; Our witness continuous function is the identity function.

 (local (defun ccr-fn1 (context x) (declare (ignore context x)) 1))
 (local (defun derivative-ccr-fn1 (context x) (declare (ignore context x)) 0))
 (local (defun ccr-fn2 (context x) (declare (ignore context x)) 1))
 (local (defun derivative-ccr-fn2 (context x) (declare (ignore context x)) 0))
 ...
 (defthm ccr-fn1-real
     (realp (ccr-fn1 context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm ccr-fn2-real
     (realp (ccr-fn2 context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm derivative-ccr-fn1-real
   (realp (derivative-ccr-fn1 context x))
   :rule-classes (:rewrite :type-prescription))

 (defthm derivative-ccr-fn2-real
   (realp (derivative-ccr-fn2 context x))
   :rule-classes (:rewrite :type-prescription))
 ...
 (defthm ccr-fn2-range-in-domain-of-fn2
     (implies (inside-interval-p x (ccr-fn2-domain))
	      (inside-interval-p (ccr-fn2 context x) (ccr-fn2-range))))
 ...
 (defun-sk forall-x-eps-delta-in-range-deriv-classical-works-for-ccr-fn1 (context x eps delta)
   (forall (x1)
	   (implies (and (inside-interval-p x1 (ccr-fn2-range))
			 ...
			 (not (equal x x1))
			 (< (abs (- x x1)) delta))
		    (< (abs (- (/ (- (ccr-fn1 context x) (ccr-fn1 context x1)) (- x x1))
			       (derivative-ccr-fn1 context x)))
		       eps)))
   :rewrite :direct)

 (defun-sk exists-delta-for-x-and-eps-so-deriv-classical-works-for-ccr-fn1 (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (ccr-fn2-range))
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-classical-works-for-ccr-fn1 context x eps delta)))))

 (defun-sk forall-x-eps-delta-in-range-deriv-classical-works-for-ccr-fn2 (context x eps delta)
   (forall (x1)
	   (implies (and (inside-interval-p x1 (ccr-fn2-domain))
			 ...
			 (not (equal x x1))
			 (< (abs (- x x1)) delta))
		    (< (abs (- (/ (- (ccr-fn2 context x) (ccr-fn2 context x1)) (- x x1))
			       (derivative-ccr-fn2 context x)))
		       eps)))
   :rewrite :direct)

 (defun-sk exists-delta-for-x-and-eps-so-deriv-classical-works-for-ccr-fn2 (context x eps)
   (exists delta
	  (implies (and (inside-interval-p x (ccr-fn2-domain))
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-classical-works-for-ccr-fn2 context x eps delta)))))

 (defthm derivative-ccr-fn1-is-derivative
  (implies (and (inside-interval-p x (ccr-fn2-range))
		(realp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-classical-works-for-ccr-fn1 context x eps))
  :hints (("Goal"
 ...
 (defthm derivative-ccr-fn2-is-derivative
  (implies (and (inside-interval-p x (ccr-fn2-domain))
		(realp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-classical-works-for-ccr-fn2 context x eps))
  :hints (("Goal"
...
(defthm ccr-fn1-differentiable-using-nonstd-criterion
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited (/ (- (ccr-fn1 context x) (ccr-fn1 context y1)) (- x y1)))
		(i-close (/ (- (ccr-fn1 context x) (ccr-fn1 context y1)) (- x y1))
			 (/ (- (ccr-fn1 context x) (ccr-fn1 context y2)) (- x y2)))))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable-using-nonstd-criterion
...
(defthm ccr-fn2-differentiable-using-nonstd-criterion
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited (/ (- (ccr-fn2 context x) (ccr-fn2 context y1)) (- x y1)))
		(i-close (/ (- (ccr-fn2 context x) (ccr-fn2 context y1)) (- x y1))
			 (/ (- (ccr-fn2 context x) (ccr-fn2 context y2)) (- x y2)))))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable-using-nonstd-criterion
...
(defun differential-ccr-fn1 (context x eps)
  (/ (- (ccr-fn1 context (+ x eps)) (ccr-fn1 context x)) eps))

(defun differential-ccr-fn2 (context x eps)
  (/ (- (ccr-fn2 context (+ x eps)) (ccr-fn2 context x)) eps))

(defun ccr-fn1-o-fn2 (context x)
  (ccr-fn1 context (ccr-fn2 context x)))

(defun differential-ccr-fn1-o-fn2 (context x eps)
  (if (equal (ccr-fn2 context (+ x eps)) (ccr-fn2 context x))
      0
      (* (differential-ccr-fn1 context (ccr-fn2 context x) (- (ccr-fn2 context (+ x eps)) (ccr-fn2 context x)))
	 (differential-ccr-fn2 context x eps))))

(defun derivative-ccr-fn1-o-fn2 (context x)
  (* (derivative-ccr-fn1 context (ccr-fn2 context x))
     (derivative-ccr-fn2 context x)))
...
(defthm ccr-fn1-o-fn2-differentiable
  (implies (and (standardp context)
		...
		(i-close x y1) (not (= x y1))
		(i-close x y2) (not (= x y2)))
	   (and (i-limited  (/ (- (ccr-fn1-o-fn2 context x) (ccr-fn1-o-fn2 context y1)) (- x y1)))
		(i-close (/ (- (ccr-fn1-o-fn2 context x) (ccr-fn1-o-fn2 context y1)) (- x y1))
			 (/ (- (ccr-fn1-o-fn2 context x) (ccr-fn1-o-fn2 context y2)) (- x y2)))))
  :hints (("Goal"
...
(defun-sk forall-x-eps-delta-in-range-deriv-ccr-fn1-o-fn2-works (context x eps delta)
  (forall (x1)
	  (implies (and (inside-interval-p x1 (ccr-fn2-domain))
			(inside-interval-p x (ccr-fn2-domain))
			(realp delta)
			...
			(not (equal x x1))
			(< (abs (- x x1)) delta))
		   (< (abs (- (/ (- (ccr-fn1-o-fn2 context x) (ccr-fn1-o-fn2 context x1)) (- x x1))
			      (derivative-ccr-fn1-o-fn2 context x)))
		      eps)))
  :rewrite :direct)

(defun-sk exists-delta-for-x-and-eps-so-deriv-ccr-fn1-o-fn2-works (context x eps)
  (exists delta
	  (implies (and (inside-interval-p x (ccr-fn2-domain))
			;(standardp x)
			(realp eps)
		   ...
		   (and (realp delta)
			(< 0 delta)
			(forall-x-eps-delta-in-range-deriv-ccr-fn1-o-fn2-works context x eps delta)))))
...
(defthmd ccr-fn1-is-differentiable
  (implies (and (standardp context)
		...
		(not (equal x1 x)))
	   (i-close (/ (- (ccr-fn1 context x) (ccr-fn1 context x1)) (- x x1))
		    (derivative-ccr-fn1 context x)))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable
				     (rdfn-classical ccr-fn1)
...
(defthmd ccr-fn2-is-differentiable
  (implies (and (standardp context)
		...
		(not (equal x1 x)))
	   (i-close (/ (- (ccr-fn2 context x) (ccr-fn2 context x1)) (- x x1))
		    (derivative-ccr-fn2 context x)))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classic-is-differentiable
				     (rdfn-classical ccr-fn2)
...
(defthm-std standard-derivative-ccr-fn1
  (implies (and (standardp x)
                (standardp context))
           (standardp (derivative-ccr-fn1 context x))))

(defthm-std standard-derivative-ccr-fn2
  (implies (and (standardp x)
                (standardp context))
           (standardp (derivative-ccr-fn2 context x))))
...
(defthmd differential-ccr-fn1-close
  (implies (and (inside-interval-p x (ccr-fn2-range))
           ...
           (equal (standard-part (differential-ccr-fn1 context x eps))
                  (derivative-ccr-fn1 context x)))
  :hints (("Goal"
           :use ((:instance close-x-y->same-standard-part
                            (x (derivative-ccr-fn1 context x))
                            (y (differential-ccr-fn1 context x eps)))
...
(defthmd differential-ccr-fn2-close
  (implies (and (inside-interval-p x (ccr-fn2-domain))
           ...
           (equal (standard-part (differential-ccr-fn2 context x eps))
                  (derivative-ccr-fn2 context x)))
  :hints (("Goal"
           :use ((:instance close-x-y->same-standard-part
                            (x (derivative-ccr-fn2 context x))
                            (y (differential-ccr-fn2 context x eps)))
...
(defthmd differential-ccr-fn1-o-fn2-close
   (implies (and (inside-interval-p x (ccr-fn2-domain))
		 ...
		 (realp eps) (i-small eps) (not (= eps 0))
		 (inside-interval-p (+ x eps) (ccr-fn2-domain)))
	    (equal (standard-part (differential-ccr-fn1-o-fn2 context x eps))
		   (derivative-ccr-fn1-o-fn2 context x)))
  :hints (("Goal"
...
(defthm expand-differential-ccr-fn1-o-fn2
  (implies (and (inside-interval-p x (ccr-fn2-domain))
                (inside-interval-p (+ x eps) (ccr-fn2-domain)))
           (equal (differential-ccr-fn1-o-fn2 context x eps)
                  (/ (- (ccr-fn1-o-fn2 context (+ x eps)) (ccr-fn1-o-fn2 context x)) eps)))
  :hints (("Goal"
...
(defthmd ccr-fn1-o-fn2-derivative-using-classical-criterion
  (implies (and (standardp context)
		...
		(standardp eps)
		(< 0 eps))
	   (exists-delta-for-x-and-eps-so-deriv-ccr-fn1-o-fn2-works context x eps))
  :hints (("Goal"
	   :by (:functional-instance rdfn-classical-derivative-using-classical-criterion
				     (rdfn ccr-fn1-o-fn2)
