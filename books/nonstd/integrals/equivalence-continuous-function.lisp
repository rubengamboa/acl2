...
(defun map-rcfn-hyper (context p)
  (if (consp p)
      (cons (rcfn-hyper context (car p))
	    (map-rcfn-hyper context (cdr p)))
    nil))

(defthm real-listp-map-rcfn-hyper
  (implies (partitionp p)
           (real-listp (map-rcfn-hyper context p))))

(defun riemann-rcfn-hyper (context p)
  (dotprod (deltas p)
	   (map-rcfn-hyper context (cdr p))))

(defthm real-riemann-rcfn-hyper
  (implies (partitionp p)
           (realp (riemann-rcfn-hyper context p))))
...
(defthm limited-riemann-rcfn-hyper-small-partition
  (implies (and (realp a) (standardp a)
		...
		(< a b))
	   (i-limited (riemann-rcfn-hyper context (make-small-partition a b))))
  :hints (("Goal"
...
(encapsulate
 ...
 (defun-std strict-int-rcfn-hyper (context a b)
   (if (and (realp a)
	    (realp b)
	    (inside-interval-p a (rcfn-hyper-domain))
	    (inside-interval-p b (rcfn-hyper-domain))
	    (< a b))
       (standard-part (riemann-rcfn-hyper context (make-small-partition a b)))
     0))
 )
...
(defthm-std realp-strict-int-rcfn-hyper
  (implies (and (realp a)
                (realp b))
           (realp (strict-int-rcfn-hyper context a b)))
  :hints (("Goal"
...
(defun int-rcfn-hyper (context a b)
  (if (<= a b)
      (strict-int-rcfn-hyper context a b)
    (- (strict-int-rcfn-hyper context b a))))

(defthm realp-int-rcfn-hyper
  (implies (and (realp a)
                (realp b))
           (realp (int-rcfn-hyper context a b))))
...
(defthm strict-int-rcfn-hyper-is-integral-of-rcfn-hyper
  (implies (and (standardp a)
		...
		(equal (car (last p)) b)
		(i-small (mesh p)))
	   (i-close (riemann-rcfn-hyper context p)
		    (strict-int-rcfn-hyper context a b)))
  :hints (("Goal"
	   :do-not-induct t
...
(defun-sk forall-partitions-riemann-sum-is-close-to-int-rcfn-hyper (context a b eps delta)
  (forall (p)
          (implies (and (<= a b)
                        (partitionp p)
                        (equal (car p) a)
                        (equal (car (last p)) b)
                        (< (mesh p) delta))
                   (< (abs (- (riemann-rcfn-hyper context p)
                              (strict-int-rcfn-hyper context a b)))
                      eps)))
  :rewrite :direct)

(defun-sk exists-delta-so-that-riemann-sum-is-close-to-int-rcfn-hyper (context a b eps)
  (exists (delta)
          (implies (and (inside-interval-p a (rcfn-hyper-domain))
                   ...
                   (and (realp delta)
                        (< 0 delta)
                        (forall-partitions-riemann-sum-is-close-to-int-rcfn-hyper context a b eps delta)))))
...
(defthm rcfn-hyper-is-integrable-hyperreal
  (implies (and (inside-interval-p a (rcfn-hyper-domain))
                ...
                (realp eps)
                (< 0 eps))
           (exists-delta-so-that-riemann-sum-is-close-to-int-rcfn-hyper context a b eps))
  :hints (("Goal"
